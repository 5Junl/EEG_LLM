...
def get_args():
    ...
    parser.add_argument('--output_dir', default='./checkpoints/clip_aligned')
...
def get_text_embeddings(labels, tokenizer, text_model, device):
    ...
    #     3: ['disgust', 'disgusting', 'repulsive', 'revolting', 'nauseating', 'unpleasant'],
    #     4: ['fear','frightening', 'scary', 'terrifying', 'fearful', 'horror'],
    #     5: ['surprise', 'surprising', 'astonishing', 'amazing', 'unexpected', 'shocking'],
    #     6: ['anger','angry', 'furious', 'enraging', 'irritating', 'infuriating']
    # }
    emotion_words = {
        0: "happy",
        1: "sad",
    ...
    # ]
    templates = ["This is EEG data recorded while watching a {} movie",]  
...
def get_test_templates():
    """Generate template texts for each emotion class"""
    emotions = {
        0: "happy",
        1: "sad",
        2: "neutral", 
    ...
    template = "This is EEG data recorded while watching a {} movie"
    texts = [template.format(e) for e in emotions.values()]
    return texts

def evaluate(eeg_encoder, text_encoder_tuple, test_loader, device):
    """Evaluate model on test set"""
    eeg_encoder.eval()
    tokenizer, text_model, eeg_projection = text_encoder_tuple
    
    # Get template embeddings
    template_texts = get_test_templates()
    inputs = tokenizer(template_texts, padding=True, return_tensors="pt").to(device)
    
    with torch.no_grad():
        template_embeddings = text_model(**inputs).last_hidden_state[:, 0, :]
        template_embeddings = F.normalize(template_embeddings, dim=1)
    ...
    # Evaluate each batch
    with torch.no_grad():
        for eeg_data, labels in tqdm(test_loader):
            eeg_data = eeg_data.float().to(device)
            eeg_data = rearrange(eeg_data, 'B N (A T) -> B N A T', T=200)
            
            # Get EEG embeddings
            eeg_embeddings = eeg_encoder(eeg_data, [0, 1, 2, 3, 7, 11, 16, 17, 18, 19, 20, 21, 22, 23, 24, 27, 28, 29, 30, 31, 32, 33, 34, 35, 38, 39, 40, 41, 42, 43, 44, 45, 46, 49, 50, 51, 52, 53, 54, 55, 56, 57, 60, 61, 62, 63, 64, 65, 66, 67, 68, 71, 72, 73, 75, 77, 78, 79, 85, 81, 82, 83, 86])
            if isinstance(eeg_embeddings, tuple):
                eeg_embeddings = eeg_embeddings[0]
            eeg_embeddings = eeg_projection(eeg_embeddings)
            eeg_embeddings = F.normalize(eeg_embeddings, dim=1)
            
            # Calculate similarities
            similarities = torch.matmul(eeg_embeddings, template_embeddings.T)
...
def train_one_epoch(eeg_encoder, text_encoder_tuple, train_loader, optimizer, criterion, device, epoch):
    ...
    print(f'Train loss for epoch {epoch+1}:  {total_loss/ len(train_loader):.4f}')
...
def main():
    ...
    # Load dataset
    train_dataset, _, test_dataset= utils.prepare_SEEDV2_dataset(
    ...
    for epoch in range(args.epochs):
        ...
        if loss < best_loss:
            ...
            clip_save_model(
                args=args,epoch="best", model=eeg_encoder,model_without_ddp= model_without_ddp)
        print(f"Epoch {epoch+1}: Test Acc = {test_acc:.2f}%, Best Acc = {best_acc:.2f}%")
